...
impl Config {
    pub fn load(repo: &Repository) -> Self {
        ...
        if config.default_base.is_none() {
            config.default_base = infer_default_base(repo);
        }
        config
    }

    fn defaults(user_path: PathBuf, repo_path: PathBuf) -> Self {
        let tools = [
            ("wt", "wt"),
            ("gh", "gh"),
            ("git", "git"),
...
fn infer_default_base(repo: &Repository) -> Option<String> {
    let output = run_capture(Command::new("git").arg("-C").arg(&repo.root).args([
        "symbolic-ref",
        "--short",
...
pub fn doctor(repo: &Repository, config: &mut Config) -> Result<(), String> {
    ...
    println!("repo config: {}", config.repo_path.display());
    println!();
